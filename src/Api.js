...
export default {
    ...
    getBarbers: async (lat=null, log=null, address=null) => {
        const token = await AsyncStorage.getItem('token');

        //console.log("LAT", lat);
        //console.group("LOG", log);
        //console.log("ADDRESS", address);

        const req = await fetch(`${BASE_API}/barbers?token=${token}&lar=${lat}&log=${log}&address=${address}`);
        const json = await req.json();
        return json;
    },
    getBarber: async (id) => {
